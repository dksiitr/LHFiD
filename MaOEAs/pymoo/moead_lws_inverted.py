...
class MOEAD_LWS(GeneticAlgorithm):
    ...
    def _initialize(self):
        ...
        inverted_ref = 1 - self.ref_dirs #DEXTER - inverted
        mat = np.array([[1/(pt+1e-6) for pt in row] for row in inverted_ref]) #DEXTER - inverted
    ...
    def _next(self):
        ...
        F_inverted = 1 - F_norm
        theta = np.array([[calc_angle(f,w) for w in self.ref_dirs] for f in F_inverted]) 
        ...
        for i in range(len(F_norm)):
            for j in range(self.pop_size):
                if theta[i,j] < self.apex_angles[j]:
                    C[i,j] = get_ws(F_norm[i],self.ws_weights[j]) #change using the matlab code in weights definition
                else:
                    C[i,j] = math.inf
