...
def survival_selection(self):
    ...
    dist_matrix = load_function('calc_perpendicular_distance')(1-F_norm, self.ref_dirs)
            ...
            if max(self.ref_dirs[i])<1: # For the non-axis reference-vectors
                ...
                for p in range(len(cluster_indices)):
                    ...
                    if ind != alpha_solution:
                        ...
                        if n_b >= n_w:
                            decide = tie_breaker(F_norm[ind], F_norm[alpha_solution], 1-self.ref_dirs[i])
                            if decide<0:
...
def get_nadir_point(self):
    ...
    for i in range(len(F)):
        ...
        for j in range(len(F)):
            if i!=j and sum(F[j]<F[i])==self.problem.n_obj:
                count+=1 
        if count == 0:
            non_dom.append(i)
    nadir_point = np.max(F[non_dom], axis=0)
    return nadir_point